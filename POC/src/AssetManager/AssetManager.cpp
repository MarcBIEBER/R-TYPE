...
sf::Texture AssetManager::getTexture(std::string id)
{
    for (auto &texture : _textures) {
        // std::cout << "texture comp: id->" << id << "| texture->" << texture.first << "|" << std::endl;
        if (texture.first == id) {
            // std::cout << "getTexture found" << std::endl;
    ...
    return texture;
    // return (_textures.count(id) > 0) ? _textures[id] : _textures["error"];
}
...
sf::Sprite AssetManager::getSprite(std::string id)
{
    for (auto &sprite : _sprites) {
        // std::cout << "sprite comp: id->" << id << "| sprite->" << sprite.first << "|" << std::endl;
        if (sprite.first == id) {
            // std::cout << "getSprite found" << std::endl;
            return sprite.second;
        }
    }
    sf::Sprite sprite;
    // std::cout << "getSprite not found" << std::endl;
    sprite.setTexture(getTexture("background"));
    return sprite;
    // return (_sprites.count(id) > 0) ? _sprites[id] : _sprites["error"];
}
...
void AssetManager::loadTexture(std::string id, std::string path)
{
    ...
    // std::cout << "Texture loaded: " << _textures.at(0) << std::endl;
}
